...
#define ServoPwmOpen 500
#define ServoPwmCLose 2500
...
/**
 * 函数：设置舵机相对角度 (±45度)
 * @param angle: 目标角度，范围 -45 到 +45 度
 */
void setServoAngleRelative(int angle) {
  // 1. 将角度限制在 -45 到 +45 的范围内
  angle = constrain(angle, -45, 45);
  
  // 2. 将 -45到+45度 映射到 500到2500微秒 的脉宽
  // 方法一：直接映射（推荐，更高效）
  int pulseWidth = map(angle, -45, 45, minPulseWidth, maxPulseWidth);
  
  /* 方法二：分步计算（易于理解）
  // 首先将角度转换为0-180度的标准范围
  // int standardAngle = angle + 45; // (-45,45) -> (0,90)
  // 然后将标准角度映射到脉宽
  // int pulseWidth = map(standardAngle, 0, 90, minPulseWidth, maxPulseWidth);
  */
  
  // 3. 将脉宽（微秒）转换为ESP8266 analogWrite所需的占空比值（0-1023）
  int duty = (pulseWidth * 1024) / pwmPeriod;
  duty = constrain(duty, 0, 1023);
  
  // 4. 输出PWM信号
  digitalWrite(ServoPwmEnablePin, HIGH); 
  analogWrite(ServoPwmPin, duty);
  delay(2000);
